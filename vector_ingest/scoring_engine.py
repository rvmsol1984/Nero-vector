...
class ServicePrincipalLoginRule(CorrelationRule):
    ...
    def evaluate(
        ...
        for ts, raw in rows:
            ...
            if app_id.lower() in self.KNOWN_SAFE_APP_IDS:
                continue
            resolved_name = app_name or self._resolve_app_name(
                app_id, tenant_id, self._db
            )
            return RuleResult(
                rule_name=self.rule_name,
                score_delta=self.SCORE_DELTA,
    ...
    @staticmethod
    def _resolve_app_name(app_id: str, tenant_id: str, db: Any) -> str | None:
        """Return a human-readable display name for *app_id*.

        Resolution order:
        1. Class-level ``_APP_NAME_CACHE`` (empty string == negative cache).
        2. Microsoft Graph ``/servicePrincipals`` endpoint.
        3. merill/microsoft-info OtherMicrosoftApps.csv fallback.

        Every result (positive or negative) is written back to the cache so
        subsequent lookups for the same app_id are free. Any network error
        is silently caught and the method returns ``None``.
        """
        import csv
        import io
        import urllib.request
        ...
        try:
            ...
            logger.debug(
                "[sp_login] graph app-name lookup failed",
                extra={"app_id": app_id},
            )

        # --- merill/microsoft-info CSV fallback ---
        try:
            csv_url = (
            ...
            for row in reader:
                ...
                if row_id == cache_key:
                    name = (
                        row.get("DisplayName") or row.get("displayName") or ""
                    ).strip()
                    if name:
                        ServicePrincipalLoginRule._APP_NAME_CACHE[cache_key] = name
                        return name
            ...
            logger.debug(
                "[sp_login] csv app-name lookup failed",
                extra={"app_id": app_id},
            )

        # Negative cache — don't retry until process restarts.
